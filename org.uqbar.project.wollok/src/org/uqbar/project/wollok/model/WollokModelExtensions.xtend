...
class WollokModelExtensions extends WMethodContainerExtensions {
	...
	def static assignments(WVariable variable) { VariableAssignmentsVisitor.assignmentOf(variable, variable.declarationContext) }
	...
	def static WMethodDeclaration method(WExpression param) {
		...
		if (container instanceof WMethodDeclaration) container
	...
	def static dispatch boolean isValidCall(WObjectLiteral c, WMemberFeatureCall call) { c.methods.exists[isValidMessage(call)] }
