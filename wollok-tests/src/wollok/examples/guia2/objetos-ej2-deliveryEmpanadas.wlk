...
object biciVieja {
	...
	method puedeLlevar(pedido, distanciaDeViaje) {
		distanciaARecorrer = distanciaDeViaje
...
object delivery {
	...
	method mejorTransporteParaLlevar(pedido, distancia) {
		...
		this.transportesQuePuedenLlevar(pedido, distancia).forEach[t | 
									if (this.desperdicio(t, pedido, distancia) < desperdicio) {
										mejorTransporte = t
									}
		]
	...
	method transportesQueNecesitoParaLlevar(pedido, distancia) {
		var transportesNecesarios = #[]
		transportes.forEach[t | if (pedido > 0) {
									if (t.puedeLlevar(pedido - t.capacidad(), distancia)) {
										transportesNecesarios.add(t)
								 		pedido -= t.capacidad()
								 	}
								}
		]
	...
	method transporteOptimo(pedido, distancia) {
		...
		var mejorTransporte = null
		transportes.forEach[t | if (this.desperdicio(t, pedido, distancia) >= 0 && this.desperdicio(t, pedido, distancia) < desperdicio) {
									desperdicio = this.desperdicio(t, pedido, distancia)
									mejorTransporte = t
								} else if (this.sobrante(t, pedido) >= 0 && this.sobrante(t, pedido) < menorSobrante) {
									menorSobrante = this.sobrante(t, pedido)
									mejorTransporte = t
								}
	...
	method transportesOptimos(pedido, mejoresTransportes, todosLosTransportes) {
		var transporte = null
		transporte = todosLosTransportes.transporteOptimo(pedido)	
		mejoresTransportes.add(transporte)
		todosLosTransportes.remove(transporte)
		pedido -= pedido - transporte.capacidad()
		if (pedido >= 0) {
	...
	method transportesOptimosPara(pedido) {
		var todosLosTransportes = transportes
		var mejoresTransportes = #[]
