...
class Asserters extends Assert {
	...
	def static InterpreterAsserter noVariables() {
		[ pair |
			...
			assertEquals("Expecting no variables but there: " 
				+ context.allReferenceNames.map[WVariable it | name].join(', '), 0, context.allReferenceNames.length
			)
		]
	}
	...
	def static InterpreterAsserter variable(String name, String value) {
		[ state |
			...
			try {
				...
				assertEquals("Expecting variable '" + name + "' to be " + value, value, context.resolve(name).toString())
			}
			catch (UnresolvableReference e) {
				Assert.fail('''Expecting variable '«name»' to be «value» But it was not resolvable in the current context: ''' + context.allReferenceNames.map[name + '=' + value].join(', '))
			}
		]
	}
