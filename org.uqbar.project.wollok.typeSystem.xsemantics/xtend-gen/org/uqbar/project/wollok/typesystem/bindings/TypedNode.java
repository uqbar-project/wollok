...
public abstract class TypedNode {
  ...
  public void fireTypeChanged() {
    ...
    final Consumer<TypingListener> _function = new Consumer<TypingListener>() {
      public void accept(final TypingListener it) {
        WollokType _type = TypedNode.this.getType();
        it.notifyTypeSet(_type);
      }
    };
    ((List<TypingListener>)Conversions.doWrapArray(_clone)).forEach(_function);
  }
}
