...
//			nameUtil.checkNewFeatureName(newName, false, status);
	...
	def isFinalFeature(JvmIdentifiableElement returnFeature) {
		return returnFeature instanceof JvmFormalParameter || (returnFeature instanceof XVariableDeclaration && !(returnFeature as XVariableDeclaration).isWriteable());
	}
	...
	def createMethodCallEdit(DocumentRewriter.Section methodCallSection, ITextRegion expressionRegion) throws BadLocationException {
		if (isNeedsReturnExpression) {
			val returnFeature = (returnExpression as XFeatureCall).feature
		...
		if (firstExpression.eContainer instanceof WMemberFeatureCall) {
			if( (firstExpression.eContainer as WMemberFeatureCall).getMemberCallArguments.size == 1) {
