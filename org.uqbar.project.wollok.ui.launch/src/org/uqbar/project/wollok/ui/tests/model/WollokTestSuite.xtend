...
interface WollokTestSuiteChild {
	def String asText()
}

@Accessors
class WollokTestSuite implements WollokTestSuiteChild {
	var List<String> fathersPath
	...
	var List<WollokTestSuite> containers = newArrayList
	var boolean processingManyFiles = false
	
	def List<WollokTestSuiteChild> children(){
			var List<WollokTestSuiteChild> children = new ArrayList()
			children.addAll(tests)
			children.addAll(containers)
			return children
	}
	
//	def boolean isChild(WollokTestSuite childContainer) {
//		val isMyChild = suiteFatherName === childContainer.suiteFatherName
//		println(suiteFatherName === childContainer.suiteFatherName)
//		println(suiteFatherName +" "+ childContainer.suiteFatherName)
//		
//		val isInOneOfMyChilds = containers.findFirst[container|container.isChild(childContainer)]
//		return isMyChild || isInOneOfMyChilds !== null
//	}
	
	def void addChild(WollokTestSuite childToAdd){
		containers.add(childToAdd)
	}
	...
	def isEmpty() {
		allTests.isEmpty && containers.isEmpty
	}
