...
class EffectStatusExtensions {
	...
	static def contextualizeChange(EffectStatus change, EffectStatus other, TypeVariable tvar) {
		...
		if (tvar.contextAffects(context)) {
			...
			val newContext = instance.programElement.context?.tvar
	...
	static dispatch def EObject context(WParameter it) { null } // Global context (?


	static def programElement(ITypeVariable it) { (owner as ProgramElementTypeVariableOwner).programElement }


	static dispatch def Boolean isGlobal(EObject it) { false }
	static dispatch def Boolean isGlobal(WNamedObject it) { true }
	static dispatch def Boolean isGlobal(WParameter it) { true }
	static dispatch def Boolean isGlobal(WVariableReference it) { ref.isGlobal }
